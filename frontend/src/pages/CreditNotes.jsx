...
import {
  Plus, X, Pencil, Trash2, Download, FileText, Package, UsersRound,
  Printer, MessageCircle, FileSpreadsheet, PenLine, RotateCcw,
} from "lucide-react";
...
export default function CreditNotes() {
  const { run: downloadCreditNotePdf, busyId } = usePdfAction();
  ...
  const sel = useBulkSelect(cnList);
  ...
  useEffect(() => {
    ...
    api.get("/invoices").then((r) => setInvoices(Array.isArray(r.data) ? r.data : (r.data?.items ?? [])));
  }, []);

  const bulkDelete = async () => {
    const { ok, failed } = await sel.runDelete(
      (id) => api.delete(`/credit-notes/${id}`),
      { reload: load }
    );
    if (failed) toast.error(`Deleted ${ok}, failed ${failed}`);
    else toast.success(`Deleted ${ok} credit note${ok === 1 ? "" : "s"}`);
  };
  ...
  const submit = async (e) => {
    ...
    setSaving(true);
    try {
      if (editingId) await api.put(`/credit-notes/${editingId}`, payload);
  ...
  // the next field anywhere in this form (customer/credit-note-details
  // fields, falling through to the grid's own Enter handling inside the
  // line-item table), Ctrl+Enter saves, Ctrl+Shift+Enter saves and
  // immediately opens a fresh blank credit note, Escape cancels.
  // Auto-focuses the Customer field the moment the modal opens — no click
  ...
  const onDelete = async (item) => {
    if (!window.confirm(`Delete ${item.credit_note_number}?`)) return;
    try {
      await api.delete(`/credit-notes/${item.id}`);
  ...
  const openEdit = (cn) => {
    setForm({
      ...
      items: (cn.items || []).map((i) => {
        const gstNum = parseFloat(i.gst_rate) || 0;
  ...
  // Global module shortcuts: Ctrl/Cmd+N → new credit note (when the list is
  // showing, not already editing one). Ctrl+Enter/Ctrl+Shift+Enter save this
  // form instead of Ctrl+S — see useEnterNavigation above — since Enter-based
  // save is the primary keyboard-first flow for a line-item voucher like
  // this one.
  useModuleShortcuts({
    onNew: () => { if (!open) openNew(); },
  });

  const customerInvoices = invoices.filter((inv) => !form.customer_id || inv.customer_id === form.customer_id);
  ...
  return (
    <div data-testid="credit-notes-page">
      <PageHeader
        eyebrow="Sales"
        title="Credit Notes"
        description="Credit notes issued to customers — sales returns, post-sale discounts, or billing corrections."
        actions={
          <PrimaryButton icon={Plus} testid="new-credit-note" onClick={openNew}>
            New Credit Note
          </PrimaryButton>
        }
      />
          ...
          <table className="w-full text-sm text-left">
            <thead className="bg-muted text-muted-foreground">
              <tr className="label-overline border-b border-border">
                ...
                <th className="px-3 py-2.5">Against Invoice</th>
            ...
            <tbody>
              {cnList.map((cn) => (
                <tr
                  ...
                  className={`border-b border-border hover:bg-muted/40 text-foreground ${sel.isSelected(cn.id) ? "bg-primary/5" : ""}`}
                >
                  <td className="px-3 py-2.5">
                    <SelectCheckbox
                      label={`Select credit note ${cn.credit_note_number}`}
                      checked={sel.isSelected(cn.id)}
                      onChange={() => sel.toggle(cn.id)}
                    />
                  </td>
                  <td className="px-3 py-2.5 font-semibold text-primary">{cn.credit_note_number}</td>
                  <td className="px-3 py-2.5 text-foreground">{cn.customer_name || "—"}</td>
                  <td className="px-3 py-2.5 text-xs text-muted-foreground">
                    {invoices.find((i) => i.id === cn.original_invoice_id)?.invoice_number || "—"}
                  </td>
                  <td className="px-3 py-2.5 text-muted-foreground">{cn.reason || "—"}</td>
                  <td className="px-3 py-2.5 text-right tabular font-semibold text-foreground">
                    {fmtMoney(cn.total, cn.currency)}
                  </td>
                  <td className="px-3 py-2.5">
                    <Badge tone={STATUS_TONE[cn.status] || "neutral"}>{cn.status}</Badge>
                  </td>
                  ...
                  <td className="px-3 py-2.5 text-xs text-muted-foreground">
                    {cn.date || (cn.created_at ? new Date(cn.created_at).toLocaleDateString() : "—")}
                  </td>
                  <td className="px-3 py-2.5">
                    <div className="flex items-center justify-end gap-1.5">
                      <button
                        onClick={() => downloadCreditNotePdf(`/credit-notes/${cn.id}/pdf`, `${cn.credit_note_number}.pdf`, cn.id)}
                        disabled={busyId === cn.id}
                        title="Download PDF"
                        data-testid={`credit-note-pdf-${cn.id}`}
      ...
      <Modal
        ...
        title={editingId ? "Edit Credit Note" : "New Credit Note"}
        icon={RotateCcw}
        size="full"
        ...
        footer={
          <div className="flex w-full flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <PrimaryButton onClick={submit} testid="save-credit-note" icon={RotateCcw} disabled={saving}
                className="bg-gradient-to-r from-[#2563EB] to-[#1D4ED8]">
                {saving ? "Saving…" : (editingId ? "Save Changes" : "Save Credit Note")}
              </PrimaryButton>
            </div>
          </div>
        }
      >
        <form ref={formRef} id="cn-form" onSubmit={submit} className="grid grid-cols-1 gap-6 xl:grid-cols-[minmax(0,1fr)_340px]">
          <div className="flex min-w-0 flex-col gap-6">
            ...
            <FormSection title="Customer" icon={UsersRound} cols={4}>
              <Field label="Customer" required>
                <PartySearch
                  ...
                  onChange={(id) => setForm({ ...form, customer_id: id, original_invoice_id: "" })}
            ...
            {/* ── Credit Note Details ───────────────────────────────────── */}
            <FormSection title="Credit Note Details" icon={FileText} cols={4}>
              <Field label="Against Invoice (optional)">
                <Select
                  value={form.original_invoice_id || ""}
                  onChange={(e) => setForm({ ...form, original_invoice_id: e.target.value })}
                >
              ...
              <Field label="Status">
                <Select value={form.status} onChange={(e) => setForm({ ...form, status: e.target.value })}>
                  {CN_STATUSES.map((s) => <option key={s} value={s}>{s}</option>)}
                </Select>
              </Field>
              <Field label="Date">
              ...
              <Field label="Reason">
                <Input value={form.reason || ""} placeholder="Sales return, discount, correction…"
                  onChange={(e) => setForm({ ...form, reason: e.target.value })} />
              </Field>
              <Field label="Remarks">
                <Input value={form.notes || ""} placeholder="Internal note"
          ...
          <aside className="flex min-w-0 flex-col gap-6">
            <div className="lg:sticky lg:top-4 flex flex-col gap-6">
              ...
              {form.original_invoice_id && (() => {
                const inv = invoices.find((i) => i.id === form.original_invoice_id);
                return inv ? (
                  <div className="rounded-2xl border border-border bg-card p-5 shadow-sm">
                    <h3 className="mb-3 font-display font-semibold text-sm text-foreground flex items-center gap-2">
                      <FileText className="w-4 h-4 text-primary" /> Against Invoice
                    </h3>
                    <div className="space-y-1.5 text-sm">
                      <div className="flex justify-between">
                        <span className="text-muted-foreground">Invoice#</span>
                        <span className="text-foreground font-semibold">{inv.invoice_number}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-muted-foreground">Original Total</span>
                        <span className="text-foreground tabular">₹{inr(inv.total)}</span>
                      </div>
                      <div className="flex justify-between">
              ...
              <SummaryCard
                ...
                total={{ label: "Credit Note Total", value: `₹${inr(grandTotal)}` }}
                footer={
                  <div className="space-y-2">
                    <div className="text-center">
                      <div className="label-overline">Credit Note Total</div>
