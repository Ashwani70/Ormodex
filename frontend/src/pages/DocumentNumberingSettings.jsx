...
// Mirror of core.document_numbering.build_document_number so the admin sees
// a live preview without a round-trip. Segment order: branch · prefix · FY · sequence.
...
export default function DocumentNumberingSettings() {
  ...
  useEffect(() => {
    api.get("/settings/document-numbering/types")
      .then(({ data }) => {
        setDocTypes(data || []);
        if (data?.length) setDocType((prev) => prev || data[0].doc_type);
      })
  ...
  const load = (type) => {
    ...
    setLoading(true);
    setLoadError(null);
    api.get(`/settings/document-numbering/${type}`)
      .then(({ data }) => {
        const b = blank();
        setForm({
          ...b,
      ...
      .catch((err) => {
        ...
        const msg = formatApiErrorDetail(null, err) || err.message || "Could not load settings.";
        setLoadError(msg);
        toast.error(msg);
      })
  ...
  const save = async (e) => {
    ...
    try {
      ...
      await api.post(`/settings/document-numbering/${docType}`, body);
  ...
  const currentLabel = docTypes.find((t) => t.doc_type === docType)?.label || "";

  return (
    <div data-testid="document-numbering-settings-page" className="space-y-6">
      <PageHeader
        eyebrow="Document Settings"
        title="Document Numbering"
        description="Choose how document numbers are generated per document type. Auto-generate builds numbers from a template; Manual lets permitted users enter their own."
      />

      <div className="max-w-3xl">
        <Field label="Document Type">
          <Select
            data-testid="doc-numbering-type"
            value={docType}
            onChange={(e) => setDocType(e.target.value)}
          >
            {docTypes.map((t) => (
              <option key={t.doc_type} value={t.doc_type}>{t.label}</option>
            ))}
          </Select>
        </Field>
      </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center gap-3 min-h-[300px] text-center">
          ...
          <PrimaryButton type="button" testid="retry-load-document-numbering" onClick={() => load(docType)}>
            Retry
          </PrimaryButton>
        </div>
      ) : (
        <form ref={formRef} onSubmit={save} className="max-w-3xl space-y-6">
          ...
          <div className="border border-border bg-card p-5" style={{ borderRadius: "var(--radius)" }}>
            <div className="label-overline mb-3">Numbering Mode — {currentLabel}</div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              >
                ...
                <p className="text-xs text-muted-foreground">Permitted users type their own number (must be unique).</p>
              </button>
            </div>
          </div>
          ...
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              ...
              <Field label="Starting Sequence">
                <Input data-testid="doc-num-start" type="text" inputMode="decimal" min="1" value={form.start_sequence}
                  onChange={(e) => set({ start_sequence: e.target.value })} />
              </Field>
              <Field label="Sequence Length (leading zeros)">
                <Input data-testid="doc-num-len" type="text" inputMode="decimal" min="1" max="12" value={form.sequence_length}
            ...
            <div className="flex items-center gap-2 pt-1 font-mono text-sm">
              ...
              <span data-testid="doc-num-preview" className="font-bold text-primary">
                {buildPreview(form)}
              </span>
            </div>
          </div>
          ...
          <div className="flex justify-end">
            <PrimaryButton type="submit" testid="save-document-numbering" disabled={saving}>
