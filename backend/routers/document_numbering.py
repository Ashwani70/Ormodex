...
async def read_settings(doc_type: str, user: dict = Depends(get_current_user)):
    ...
    settings = await dn.get_settings(doc_type)
    return {**settings,
...
async def write_settings(doc_type: str, payload: DocumentNumberingSettings, user: dict = Depends(require_admin)):
    ...
    return await dn.save_settings(doc_type, payload.model_dump(), user=user)


@router.get("/{doc_type}/preview")
async def preview_number(doc_type: str, user: dict = Depends(get_current_user), seq: Optional[int] = None):
